...
But in the real world, its not possible to take lots of samples over and over again - and we generally just take one. This time, we observe `r k` heads. This is equivalent to creating a sample of size $`r n`$ and observing a mean probability of $`r sample_mean`$.
...
plot_ly(type = 'scatter', mode = 'lines') %>%
  add_trace(x = rep(c(one_time_sample.conf_02.5, one_time_sample.conf_97.5), each = 3), y = c(1,-1,0,0,-1,1), name = 'Cumulative mean with sampling') %>%
  add_trace(x = p, y = 0, mode = 'markers', name = 'Cumulative mean with sampling', marker = list(size=50, symbol = 'x')) %>%
  layout(showlegend = F
         ...
         , title = "Sample confidence interval either contains true value of theta or not")
...
$$
$$
\text{Likelihood: } \text{Beta}[k_{2},n_{2}-k_{2}] = \text{Beta}[3,10-3] = \text{Beta}[3,7]
$$

$$
\text{Posterior: } \text{Beta}[k_{1}+k_{2},n_{1}-k_{1}+n_{2}-k_{2}] = \text{Beta}[5+3,5+7] = \text{Beta}[8,12]  
$$
``` {r, echo = F, fig.width=10, fig.height=7}
k1 <- 5
n1 <- 10
