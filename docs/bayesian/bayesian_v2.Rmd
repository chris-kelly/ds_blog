...
So - let's say the time we tried flipping `r n` times, we got heads `r p*n` times (our prior) and the new coin flipped heads `r k` times (our new evidence). We can then get the following equations:
...
In other words, now we have incorporated our prior data and our new evidence, we have a likelihood of observing $`r k`$ heads next time we sample $`r n`$ times of $`r posterior[k+1]`$, and a likelihood of observing $`r k1`$ heads next time of $`r posterior[k1+1]`$. The most likely result is $`r which.max(posterior)-1`$ with a probability of $`r max(posterior)$`.
...
prior <- dbeta(x, k1+1, n1-k1+1)/(n1+1)
likelihood_f <- dbeta(x, k2+1, n2-k2+1)/(n2+1)
posterior <- dbeta(x, k1+k2+1, n1-k1+n2-k2+1)/(n1+1)
...
Now why is it an integral? Let's think of the coin example above where there are only two types of coins in the economy. To predict the likelihood of getting heads next time, given our evidence, we do the following:
...
If we chose a purely frequentist approach, we would have guessed that the likelihood is $`r k/n`$, since we observed $`r k`$ heads, and \theta can only take one value. If we chose to only use our prior informtion that 90% of coins were fair, we would have guessed $0.9 \times 0.5 + 0.1 \times `r k/n` = `r 0.9*0.5+0.1*k/n`$. However, since we have incorporated both the prior information and the new evidence, we predict a result that is somewhere in the middle, at $`r round(0.5*fair_coin_prob+0.2*biased_coin_prob,4)`$. 
...
And by taking into account all the possible values that theta can take, we are essentially integrating by theta. 
